...
TW2812LEDS::TW2812LEDS ( S_W2812MOSI_T m, uint16_t w, uint16_t h) : c_width (w), c_height (h), c_leds_amount (w*h)
{
	pinmosi = m;
	hardconstruct ();
}
...
TW2812LEDS::TW2812LEDS (S_W2812MOSI_T m, uint32_t cnt) : c_width (0), c_height (0), c_leds_amount (cnt)
{
	pinmosi = m;
	hardconstruct ();
}
...
{
	const uint16_t c_rawcnt = raw_size ();
	const uint8_t c_bit_lo = 8;
	const uint8_t c_bit_hi = 2;
	uint16_t ix = 0;

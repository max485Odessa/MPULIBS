...
#define ZEDRTK_MODULE 1



// GPS_AUTO_CONFIG	было 1 поставил 0		0:Disables automatic configuration 1:Enable automatic configuration	Controls if the autopilot should automatically configure the GPS based on the parameters and default settings
// GPS_TYPE2	2		0:None 1:AUTO 2:uBlox 3:MTK 4:MTK19 5:NMEA 6:SiRF 7:HIL 8:SwiftNav 9:UAVCAN 10:SBF 11:GSOF 13:ERB 14:MAV 15:NOVA 16:HemisphereNMEA	GPS type of 2nd GPS
...
#define C_RESETGPS_N_PIN GPIO_PIN_0
...
//#define C_GPSRECVBUF_SIZE 4096		// размер буфера под полный пакет данных, меняем под нужный GPS для оптимизации оперативной памяти
...
const char C_GNSS_MAXFIELD = EGNSSINX_D_ELIPS;
...
const char C_GGA_MAXFIELD = EGGAINDX_D_ELIPS;
...
const char C_RMC_MAXFIELD = ERMCINDX_MAGEW;
	

enum EGSAINX {EGSAINX_FIXMOD = 2, EGSAINX_PDOP = 15, EGSAINX_HDOP = 16, EGSAINX_VDOP = 17};
const char C_GSA_MAXFIELD = EGSAINX_VDOP;
...
typedef struct {
	...
	unsigned char ReservedforITRFRealizationYear;
	bool GPSIndicator;
	bool GLONASSIndicator;
	bool ReservedforGalileoIndicator;
	bool ReferenceStationIndicator;
  double AntennaReferencePointECEF_X;
	bool SingleReceiverOscillatorIndicator;
	bool Reserved1;
  double AntennaReferencePointECEF_Y;
	bool Reserved2;
	bool Reserved3;
  double AntennaReferencePointECEF_Z;
} RTCM3_1005MESSAG;
...
#define C_RTCM3MESSAGE_AMOUNT 4
#define C_ROVERMESSAGE_AMOUNT 2  // 6
#define C_RTKQGROUND_AMOUNT 7
...
const unsigned long C_CFG_UART1OUTPROT_RTCM3X = 0x10740004;						// L - rtcm3 messages to uart 1  разрешено по умолчанию
const unsigned long C_CFG_UART1INPROT_RTCM3X = 0x10730004;						// input enable RTCM3X разрешено по умолчанию
// CFG-MSGOUT-*
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1005_UART1 = 0x209102be;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1074_UART1 = 0x2091035f;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1077_UART1 = 0x209102cd;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1084_UART1 = 0x20910364;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1087_UART1 = 0x209102d2;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1094_UART1 = 0x20910369;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1097_UART1 = 0x20910319;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1124_UART1 = 0x2091036e;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1127_UART1 = 0x209102d7;	// u1
const unsigned long C_CFG_MSGOUT_RTCM_3X_TYPE1230_UART1 = 0x20910304;	// u1
const unsigned long CFG_MSGOUT_UBX_RXM_RTCM_UART1 = 0x20910269;				// u1
...
// UBX PVT используется для получения точности


//const unsigned long RTKKEYARRAY_RTCMOUT[C_RTCM3MESSAGE_AMOUNT] = {C_CFG_MSGOUT_RTCM_3X_TYPE1005_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1077_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1087_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1097_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1127_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1230_UART1};
const unsigned long RTKKEYARRAY_QGROUNDUBX[C_RTKQGROUND_AMOUNT] = {C_CFG_MSGOUT_UBX_NAV_SVIN_UART1, C_CFG_MSGOUT_UBX_NAV_PVT_UART1, C_CFG_MSGOUT_UBX_NAV_SAT_UART1, C_CFG_MSGOUT_UBX_NAV_STATUS_UART1, C_CFG_MSGOUT_UBX_NAV_DOP_UART1, C_CFG_MSGOUT_UBX_NAV_VELNED_UART1, C_CFG_MSGOUT_UBX_MON_RF_UART1};
//const unsigned long RTKKEYARRAY_RTCMOUT[C_RTCM3MESSAGE_AMOUNT] = {C_CFG_MSGOUT_RTCM_3X_TYPE1005_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1074_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1077_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1084_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1087_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1094_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1097_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1124_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1127_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1230_UART1};
...
const unsigned long RTKROVERENABLEARRAY[C_ROVERMESSAGE_AMOUNT] = {C_CFG_MSGOUT_UBX_NAV_PVT_UART1,  CFG_MSGOUT_NMEA_ID_GNS_UART1};


//const unsigned long RTKKEYARRAY_RTCMOUT[C_RTCM3MESSAGE_AMOUNT] = {C_CFG_MSGOUT_RTCM_3X_TYPE1005_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1074_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1084_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1094_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1124_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1230_UART1};


//const unsigned long RTKKEYARRAY_RTCMOUT[C_RTCM3MESSAGE_AMOUNT] = {C_CFG_MSGOUT_RTCM_3X_TYPE1074_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1084_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1094_UART1, C_CFG_MSGOUT_RTCM_3X_TYPE1124_UART1};
...
enum EGPSMESSTYPE {EGPSMESSTYPE_NONE = 0, EGPSMESSTYPE_UBX = 1, EGPSMESSTYPE_NMEA = 2, EGPSMESSTYPE_RTCM = 3 };
enum EGPSSW {EGPSSW_SPEED_DETECT_A = 0, EGPSSW_WORK, EGPSSW_RESET, EGPSSW_RESET_WAIT, EGPSSW_ENDENUM = 4};
...
//enum EUSARTSPD {EUSARTSPD_38400 = 0,  EUSARTSPD_460800 = 1, EUSARTSPD_MAX = 2};
...
//const unsigned long C_USARTSPEED_INDX[EUSARTSPD_MAX] = {38400,460800};
const unsigned long C_SPEEDSET_TIMEOUT = 1100;			// период поиска любых пакетов с нормальной CRC на текущей скорости
const unsigned long C_GPS_NODATA_TIMEOUT = 2000;		// если с GPS в течении 2-3 секунд нет данных с нормальной CRC - переходим к детектированию скорости
...
enum EUBXREQ {ENEXTREQ_DOP = 0, ENEXTREQ_PVT = 1, ENEXTREQ_RATE = 2, ENEXTREQ_RTKMODE = 3, ENEXTREQ_END = 4};
...
const utimer_t C_PERIOD_PVT = 2000;
...
#define C_NMEA_LINE_TIMEOUT 3000
#define C_NMEA_UTC_TAG_TIMEOUT 3000
//#define C_RAW_BETWEENPACKETS_TIMEOUT 3000
#define C_GPSBUF_SIZE (4096+2048)

class TCANARDGPS: public TUSART_IFACE {
	private:

    static char linebuf[C_GPSBUF_SIZE];
    static uint32_t push_ix;
    //const uint32_t C_GPS_LINEBUF_SIZE;
	
		volatile utimer_t DopsTimer;
		volatile utimer_t ErrorTimeout;
		// Таймеры периодических отправок
		volatile utimer_t UBXReqTimet_DOP;
		volatile utimer_t UBXReqTimet_PVT;
		volatile utimer_t UBXReqTimet_RATE;
		volatile utimer_t UBXReqTimet_RTK;
		volatile static utimer_t Timer_RTK_implement;
	
		volatile utimer_t Timer_UTC_value;
		float utc_last_seconds_value;
		...
		static void NVIC_USART_ENABLE (bool stat);
							
		bool f_new_gps_data;														// по этому флагу определяется начало и завершение gps цикла сообщений
		bool f_new_aux_data;
		...
		void task_rx ();
		uint32_t loc_cnt_nmea;
		uint32_t loc_cnt_messages;
    bool check_nmea_linedata (uint32_t &ix_start, uint32_t &mess_size, uint32_t last_psh_ix);
    bool check_ubx_linedata (uint32_t &ix_start, uint32_t &mess_size, uint32_t last_psh_ix);
    bool check_rtcm_linedata (uint32_t &ix_start, uint32_t &mess_size, uint32_t last_psh_ix);
		
		void cb_ubx_data (HDRUBX *inp, uint32_t size);
		bool cb_nmea_data (uint8_t *inp, uint32_t size);
		void cb_rtcm3_data (uint8_t *inp, uint32_t size);
	...
	protected:
		...
		void gps_raw_reset_pin (bool val);
		
	
	public:
		TCANARDGPS ();
		//TCANARDGPS (uint16_t rxbufsize);
	
		void Init (unsigned long speed_set);			
		static void DeInit ();
		void Task ();
	
		bool check_fix_data ();																			// проверить есть ли данные gps fix сообщения
		bool check_aux_data ();																			// проверить есть ли данные aux сообщения
		bool GetFixData (uavcan_equipment_gnss_Fix &datt);						// получить распарсенные данные
		bool GetAuxData (uavcan_equipment_gnss_Auxiliary &datt);
		TCOVRPARAMS *GetCovariances ();
		void clr_fix_flag ();
		void clr_aux_flag ();
	
		//void SetRTKStationMode (bool val_mode);
		void SetRTKStation_Lat (unsigned long lat_par);
		void SetRTKStation_Lon (unsigned long lon_par);
	
		static uint8_t GetCurentSat_count ();
		static uint8_t GetDimensModeGPS ();
		static bool CheckRTCM_Format (HDRTRANPLAYRTCM3 *lframe, unsigned long *cut_sz);						// проверяет входной буфер на соответствие формату RTCM3 и возвращает размер записи
	
		bool GetInjectRTCMBufer (TBUFPARAM *lprm);	// проверяет готовность буфера к приему новых rtcm данных
		bool TransmitRTCM (TBUFPARAM *ltxprm);
		static bool GetRTK_implement_status ();
		
		void gps_hardware_reset ();		// делает холодный старт

  ...
  uint8_t CANARD_MAYBE_UNUSED(root_item))
{
    ...
    canardEnSclr(msg_buf, offset, 3, (void*)&source->gnss_time_standard); // 7
    //offset += 3;

    // Void5
    offset += 5;
    canardEnSclr(msg_buf, offset, 8, (void*)&source->num_leap_seconds); // 255
    //offset += 8;

    source->longitude_deg_1e8 = CANARD_INTERNAL_SATURATE(source->longitude_deg_1e8, 68719476735)
    canardEnSclr(msg_buf, offset, 37, (void*)&source->longitude_deg_1e8); // 68719476735
    //offset += 37;

    source->latitude_deg_1e8 = CANARD_INTERNAL_SATURATE(source->latitude_deg_1e8, 68719476735)
    canardEnSclr(msg_buf, offset, 37, (void*)&source->latitude_deg_1e8); // 68719476735
    //offset += 37;

    source->height_ellipsoid_mm = CANARD_INTERNAL_SATURATE(source->height_ellipsoid_mm, 67108863)
    canardEnSclr(msg_buf, offset, 27, (void*)&source->height_ellipsoid_mm); // 67108863
    //offset += 27;

    source->height_msl_mm = CANARD_INTERNAL_SATURATE(source->height_msl_mm, 67108863)
    canardEnSclr(msg_buf, offset, 27, (void*)&source->height_msl_mm); // 67108863
    //offset += 27;
    ...
    for (c = 0; c < 3; c++)
    {
        ...
        canardEnSclr(msg_buf, offset, 16, (void*)&tmp_float); // 32767
        //offset += 16;
    }

    source->sats_used = CANARD_INTERNAL_SATURATE_UNSIGNED(source->sats_used, 63)
    canardEnSclr(msg_buf, offset, 6, (void*)&source->sats_used); // 63
    //offset += 6;

    source->status = CANARD_INTERNAL_SATURATE_UNSIGNED(source->status, 3)
    canardEnSclr(msg_buf, offset, 2, (void*)&source->status); // 3
    //offset += 2;
    ...
    // - Add array length
    canardEnSclr(msg_buf, offset, 4, (void*)&source->position_covariance.len);
    //offset += 4;
    ...
    {
        // - Add array length
        canardEnSclr(msg_buf, offset, 4, (void*)&source->velocity_covariance.len);
        //offset += 4;
    }

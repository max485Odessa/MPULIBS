...
// uavcan ID формат:  
//  31: - extended bit (метка что 29 битный индификатора) 21.11.19
//  30: - (1 - remove transmition, 0 - data) 21.11.19
//  29-0	(адресный индификатор) 21.11.19
...
# error "RIR bits do not match TIR bits, TIR --> libcanard conversion is not possible"
    ...
    {
			// в случае если CAN с 11 битным индификатором  CAN_A  21.11.19
        out = (CANARD_CAN_STD_ID_MASK & (id >> 21U));
    }
    else
    {
			// в случае если CAN с 29 битным индификатором  CAN_B  21.11.19
...
// INAK бит - сигнализирует вхождение в режим инициализации, работает в паре с регистром запроса INRQ (установка INAK запаздывает во времени, но повторяет уровень INRQ)  20.11.19
                        ...
                        const CanardSTM32IfaceMode iface_mode)
{
    ...
    if (!waitMSRINAKBitStateChange(BXCAN, true))                // Wait for synchronization (ожидать переключения в режим инициализации)  11.19
    {
     ...
     */
    BXCAN->MCR = CANARD_STM32_CAN_MCR_ABOM | CANARD_STM32_CAN_MCR_AWUM | CANARD_STM32_CAN_MCR_INRQ;  // RM page 648  674(RM0008 Rev 20)   11.19
    ...
    if (!waitMSRINAKBitStateChange(BXCAN, false))		// ожидать выхода из режима инициализации
    {
        BXCAN->MCR = CANARD_STM32_CAN_MCR_RESET;
     ...
     */
    {
        uint32_t fmr = CANARD_STM32_CAN1->FMR & 0xFFFFC0F1U;							// обнулить CAN2SB по маске  .11.19ER  - должно 0xFFFFC0FFU
        fmr |= CANARD_STM32_NUM_ACCEPTANCE_FILTERS << 8U;                	// CAN2 start bank = 14 (if CAN2 is present)
        CANARD_STM32_CAN1->FMR = fmr | CANARD_STM32_CAN_FMR_FINIT;				// входим в режим програмирования фильтров
    }
...
int16_t canardSTM32Transmit(const CanardCANFrame* const frame)
{
    ...
    uint8_t tx_mailbox = 0xFF;
		...
		/* CAN transmit status register (CAN_TSR)
			TME0,TME1,TME2 - This bit is set by hardware when no transmit request is pending for mailbox
			// - этот бит/биты устанавливается когда нет запросов на передачу с соответствующего mailbox(0,1,2) - тоесть когда свободны 21.11.19
		*/

    static const uint32_t AllTME = CANARD_STM32_CAN_TSR_TME0 | CANARD_STM32_CAN_TSR_TME1 | CANARD_STM32_CAN_TSR_TME2;	
		uint32_t cur_tsr = BXCAN->TSR;				// 21.11.19 замораживание аппаратного значения
		uint32_t tmp_d = cur_tsr & AllTME;   // 21.11.19 свободные mailbox
    if (tmp_d != AllTME)                // At least one TX mailbox is used, detailed check is needed 21.11.19
			{
			if (tmp_d) // 21.11.19 какой-то свободен
				{
        const bool tme[3] =
					{
            (cur_tsr & CANARD_STM32_CAN_TSR_TME0) != 0,
            (cur_tsr & CANARD_STM32_CAN_TSR_TME1) != 0,
            (cur_tsr & CANARD_STM32_CAN_TSR_TME2) != 0
					};
        ...
        for (uint8_t i = 0; i < 3; i++)
            ...
            if (tme[i])                                 // This TX mailbox is free, we can use it
							{
                tx_mailbox = i;
								break;					// 21.11.19 - добавил break;  - найден свободный ящик - выйти с поиска
							}
        ...
        if (tx_mailbox == 0xFF)
            ...
            return 0;
				...
				{
				return 0;			// все ящики заняты   21.11.19
				}
    }
    else                                                // All TX mailboxes are free, use first
    {
				// все ящики свободны - значит спокойно можно использовать 0
        tx_mailbox = 0;
    }
    ...
    mb->TDHR = (((uint32_t)frame->data[7]) << 24U) |			// 4 байта данных 21.11.19
               (((uint32_t)frame->data[6]) << 16U) |
               (((uint32_t)frame->data[5]) <<  8U) |
               (((uint32_t)frame->data[4]) <<  0U);
    mb->TDLR = (((uint32_t)frame->data[3]) << 24U) |			// еще 4 байта данных 21.11.19
...
// при приеме извлекаются данные 8 байт, размер принятых данных и ID 
int16_t canardSTM32Receive(CanardCANFrame* const out_frame)
{
    ...
    /*
     * Reading the TX FIFO    опечатка, нужно RXFIFO 21.11.19 !!!
     */
    for (uint_fast8_t i = 0; i < 2; i++)
    {
        volatile CanardSTM32RxMailboxType* const mb = &BXCAN->RxMailbox[i];
				cur_RFxR = *RFxR[i];			// фиксируем значение регистра fifo  21.11.19
        if ((cur_RFxR & CANARD_STM32_CAN_RFR_FMP_MASK) != 0)		// проверка количества сообщений в буфере fifo  21.11.19
        {
            if (cur_RFxR & CANARD_STM32_CAN_RFR_FOVR)
            {
                                              ...
                                              const uint8_t num_filter_configs)
{
     ...
     */
    CANARD_STM32_CAN1->FA1R = 0;			// отключение блока фильтрации адресов (всех)
    ...
    for (uint8_t i = 0; i < num_filter_configs; i++)
    {
        ...
        if ((cfg->id & CANARD_CAN_FRAME_EFF) || !(cfg->mask & CANARD_CAN_FRAME_EFF))	// проверка на указание extended 2.0B (29 id)
					{
            ...
            id |= CANARD_STM32_CAN_RIR_IDE;
					...
					{
					// CAN2.0A  - 11 битный индификатор
            id   = (cfg->id   & CANARD_CAN_STD_ID_MASK) << 21U;
            mask = (cfg->mask & CANARD_CAN_STD_ID_MASK) << 21U;
					}
				// перестановки битов RTR(remote or data) и EFF (extended adress CAN2.0 А или B)
        ...
        CANARD_STM32_CAN1->FA1R |= 1U << filter_index;      // Enable  включение блока фильтрации адрессов
    }
    return 0;
}
...
// написал функцию статуса занятости блока передачи  21.11.19
// в случае передачи или наличия очереди в TXFIFO возвращает TRUE
// возвращает статус занятости блока передачи CAN (в процессе передачи или имеются данные для передачи)
bool canardSTM32_TxStatus ()
{
	bool rv = true;			// занято
	static const uint32_t AllTME = CANARD_STM32_CAN_TSR_TME0 | CANARD_STM32_CAN_TSR_TME1 | CANARD_STM32_CAN_TSR_TME2;	
	uint32_t cur_tsr = BXCAN->TSR;	
	if ((cur_tsr & AllTME)) rv = false;
	return rv;
}
...
// написал функцию статуса занятости блока передачи  21.11.19
// блок имеет принятые данные
bool canardSTM32_RxStatus ()
{
	bool rv = false;
	if ((BXCAN->RF0R & CANARD_STM32_CAN_RFR_FMP_MASK) || (BXCAN->RF1R & CANARD_STM32_CAN_RFR_FMP_MASK)) rv = true;		// есть данные в буфере fifo
	return rv;
}

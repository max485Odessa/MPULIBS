...
// Добавления из версии 0.2			-----------------  // 19.2.21		

typedef struct CanardBufferBlock
{
    struct CanardBufferBlock* next;
    uint8_t data[1];				
} CanardBufferBlock;						// 19.2.21							 
													 
													 
													 
struct CanardRxTransfer					// 19.2.21		
{
    /**
...
uint16_t canardConvertNativeFloatToFloat16(float value);  // 19.2.21		
float canardConvertFloat16ToNativeFloat(uint16_t value);  // 19.2.21		
void canardEnSclr(void* destination, uint32_t &bit_offset, uint8_t bit_length, const void* value);  // 19.2.21		
void canardEncodeScalar(void* destination,      ///< Destination buffer where the result will be stored
                        ...
                        const void* value);     ///< Pointer to the value; see the table  // 19.2.21	
...
# define CANARD_ASSERT(x)   //assert(x)   21.11.19
#endif

 ...
 *
 * Source file: E:\DSDL\libcanard\dsdl_compiler\pyuavcan\uavcan\dsdl_files\uavcan\protocol\341.NodeStatus.uavcan
 */
...
{
    canardEnSclr(msg_buf, offset, 32, (void*)&source->uptime_sec); // 4294967295
    //offset += 32;

    source->health = CANARD_INTERNAL_SATURATE_UNSIGNED(source->health, 3)
    canardEnSclr(msg_buf, offset, 2, (void*)&source->health); // 3
    //offset += 2;

    source->mode = CANARD_INTERNAL_SATURATE_UNSIGNED(source->mode, 7)
    canardEnSclr(msg_buf, offset, 3, (void*)&source->mode); // 7
    //offset += 3;

    source->sub_mode = CANARD_INTERNAL_SATURATE_UNSIGNED(source->sub_mode, 7)
    canardEnSclr(msg_buf, offset, 3, (void*)&source->sub_mode); // 7
    //offset += 3;

    canardEnSclr(msg_buf, offset, 16, (void*)&source->vendor_specific_status_code); // 65535
    //offset += 16;

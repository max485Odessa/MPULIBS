...
class ICM42688: public TI2CTRDIFACE {
 public:
	enum EIMUSW {EIMUSW_NONE = 0, EIMUSW_INIT, EIMUSW_READ, EIMUSW_ERROR, EIMUSW_COMPLETE, EIMUSW_ENDENUM};
     ...
     * @return     ret < 0 if error
     */
	bool setGyroODR(ODR odr);
	int getGyroODR();

	bool setFilters(bool gyroFilters, bool accFilters);
	...
	bool   computeOffsets();
	bool   setAllOffsets();                    //Set all Offsets computed
	bool   setGyrXOffset(int16_t gyrXoffset);  //#TODO add the getOffset function
	bool   setGyrYOffset(int16_t gyrYoffset);
	bool   setGyrZOffset(int16_t gyrZoffset);
	bool   setAccXOffset(int16_t accXoffset);
	bool   setAccYOffset(int16_t accYoffset);
	bool   setAccZOffset(int16_t accZoffset);
	float getAccelRes();
	float getGyroRes();
	int   setUIFilterBlock(UIFiltOrd gyroUIFiltOrder, UIFiltOrd accelUIFiltOrder);
	bool   setGyroNotchFilter(float gyroNFfreq_x, float gyroNFfreq_y, float gyroNFfreq_z, GyroNFBWsel gyro_nf_bw);  //
	int   selfTest();
	int   testingFunction();
 ...
 protected:
	...
	*/
	...
	bool readRegisters(uint8_t subAddress, uint8_t count, uint8_t* dest);
